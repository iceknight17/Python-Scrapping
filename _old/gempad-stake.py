...
stakeUrl = 'https://gempad.app/api/stake/stakesV2?tab=0&networks=1,56,42161,8453,137,204,3797,179,7171,25186,6661,1116,88&page=XXX&search=&status=0,1,2,3&badge=none,kyc,vetted,audit&sort=nosort'

pageFrom = int(sys.argv[1])
pageTo = int(sys.argv[2])

for page in tqdm(range(pageFrom, pageTo + 1)):
    data = requests.get(stakeUrl.replace("XXX", str(page))).json()
    if data['pools']:
        tempPools = [x['ipfs']['telegram'] for x in data['pools']]
        results.append(tempPools)

with open('gempad-stakes-result.json', 'w') as file:
