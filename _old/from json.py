...
url = "https://coindiscovery.app/coin/xxx/overview"
...
for link in tqdm(links):
    soup = BeautifulSoup(requests.get(url.replace("xxx", link)).text, "html.parser")
    linkEle = soup.select_one(".chat-dropdown a")
    if linkEle:
        telLink = linkEle.get('href')
        if telLink == "#":
            dropdownEle = soup.select_one(".chat-dropdown a.dropdown-item")
            if dropdownEle and dropdownEle.get('href'):
                results.append(dropdownEle.get('href'))
        else:
            results.append(telLink)
